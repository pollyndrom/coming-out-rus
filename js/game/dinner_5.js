...
function Start_Dinner_5(){
	...
	Choose({
		...
		"Это моё ИСКУССТВО!": function(message){
			...
			f("Знаешь, следующее, к чему ты придёшь - это написание плохой поэзии, или чего-нибудь ещё.");
			n(". . .");
			Casual();
		}
	});
...
function Casual_2(){
	...
	Choose({
		...
		"ПАПА Я БИСЕКСУАЛ И ТРАХАЛ ДЖЕКА.": function(message){
			...
			m("БИАТЛОНИСТ!");
            m("Пока ещё не совсем...но когда-нибудь потом может им стать... ");
            m("Кроме того, Ник будет ездить на велосипеде в школу каждый день, начиная со следующей недели.");
			f("О, Боже!");
			f("Ты мог бы, конечно, немного похудеть, а иначе как ты заполучишь девушку?");
			f("Ты, должно быть, унаследовал свою пухлость от матери. Хаха!");
			n("Ха ха.");
			m("Говоря про школу...");
			Getting_A_Tutor();
		}
	...
	switch($.promise_silence){
		case "yes":
			...
			if($.tried_talking_about_it){
				m("Ты <i>только что</i> пытался заговорить про...");
			}
			break;
		case "no":
		...
		case "tit for tat":
			...
			if($.tried_talking_about_it){
				m("Ты <i>только что</i> пытался рассказать это.");
			}
			break;
	}
