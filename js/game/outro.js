...
function Closure(){
	...
	p("Тьфу.");
	p("Я чувствую себя отвратительно, просто используя те же цветные диалоговые шары, что и персонаж отца.");

	Show("nicky","coffee_nicky_still");
	Show("cup","cup_steam");

	N("Что напоминает мне: многие персонажи были заменены.");
	N("Все имена были изменены, кроме моего.");
	N("Я полностью исключил моего младшего брата, потому что он невиновен.");
	N("И я вернул своего отца, хотя он покинул семью задолго до 2010 года.");
	...
	Choose({
		...
		"Дай угадать, It Gets Better&trade?": function(message){
			p(message);
...
function Closure_Story(){
	...
	if($.coming_out_stories_left==3){
		...
		N("Не переживай, ты услышишь все три из них.");
	}else if($.coming_out_stories_left==2){
		N("А теперь, какую версию ты хотел бы услышать дальше?");
	...
	if(!$.told_story_half_truth) options["Наполовину правда."]=Tell_Me_A_Half_Truth; 
	Choose(options);
...
function Is_Last_Story(){
	if($.coming_out_stories_left==0){
		if($.asked_about && $.asked_credits){
			p("Вот снова ты делаешь единственную опцию, на которую можно нажать...");
		}else{
			p("Почему ты даёшь выбрать единственную опцию, когда она одна?");
...
function Tell_Me_A_Lie(message){
	...
	if($.im_a_poet){
		N("Я бродил по Большому Белому Северу. Поддерживая себя, сочиняя любительские стихи для незнакомцев.");
	}else{
		N("Я бродил по Большому Белому Северу. Поддерживая себя, делая неинтересные веб-игры.");
	}
	N("Я ел цветы. Следил за радугой. И подружился с гомосексуальным единорогом.");
	p(". . .");
	N("В конце концов я добрался до Аляски, где я встретил взрослую бисексуальную пару по имени Бонни и Клайд.");
	N("Бонни была пумой в середине 30-х, а Клайд был человеком-пантерой начала 40-х.");
	...
	Choose({
		...
		"...\"человек-пантера\".": function(message){
			$.outro_convo_lie = 3;
			p(message);
			N("Также известный как пидор-ягуар.");
			Tell_Me_A_Lie_2();
		}
	});
}
function Tell_Me_A_Lie_2(){
	...
	N("Они приняли меня как своего приёмного ребёнка, а я был их мальчиком на полную ставку.");

	if($.outro_convo_lie==1){
		p("...Еще раз спасибо за твою, эм, гибкость.");
	}
	...
	N("Они воспитали меня, показали мне любовь, и я вырос, чтобы стать продуктивным членом общества.");

	switch($.outro_convo_lie){
		case 2: p("И когда ты увеличиваешь масштаб этого фрактала, возникает БОЛЬШЕ свихнутости."); break;
		case 3: p("...\"ЧЕЛОВЕК-ПАНТЕРА\"."); break;
	}
...
function Tell_Me_A_Truth(message){
	...
	N("Я принял совет Джека и пародировал Начало в своей 'странной веб-игре': “Reimagine :The Game”.");
	switch($.inception_answer){
		case "awake": N("Я не сказал, что Коббс проснулся в конце."); break;
	...
	N("Игра Reimagine :The Game получила интернет-известность! Хороший кусок моего портфолио.");
	...
	Choose({
		...
		"В заливе Сан-Франциско дружелюбны к ЛГБТ.": function(message){
			...
			p("Эм.. никто не называет его так.");
			Tell_Me_A_Truth_2();
		},
		"О, я люблю EA!  Они создали The Sims, верно?": function(message){
			$.outro_convo_truth = 1;
			p(message);

			N("Ага! Но я над этим не работал. Наша команда делала веб-версию игры--");
			N("[БУКВАЛЬНО НЕ МОЖЕТ РАСКРЫТЬ]");
			p("Эм.");
			Tell_Me_A_Truth_2();
		}
	});
...
function Tell_Me_A_Truth_2(){
	...
	N("Но я оставался на связи с друзьями в EA и остался в районе залива.");
...
function Tell_Me_A_Half_Truth(message){
	...
	N("Мы рассказали друг другу об этом во время учебной сессии по  предмету "+$.studying_subject+" ");
	...
	Choose({
		...
		"Вы в конечном итоге, замутили вместе секс?": function(message){
...
function Tell_Me_A_Half_Truth_2(){
	...
	N("Типа... дать знак 'подойди сюда' движением своих пальцев, или потереть голову против шеи.");
	p("Слишком много информации, чувак...");

	if($.changing_schools || !$.father_oblivious){
		N("В конце концов, я перешел в ее школу.");
	}

	N("Мы были лучшими друзьями и до сих пор дружим! Мы оба переехали в США, далеко от наших ненавистных семей.");
	N("Вместе мы помогли друг другу преодолеть нашу неуверенность и выяснить, кем мы были...");
	N("Гордые бисексуальные шлюшки.");

	p("Какая трогательная история. Я думаю.");
...
function Finale_2(){
	...
	if($.punched){
		N("Мои тексты читают. Был вынужден менять школу. Получил удар в лицо.");
	}else if($.father_oblivious==false){
		N("Мои тексты читают. Был вынужден менять школу. Все словесные оскорбления.");
	}else if($.changing_schools){
		N("Мои тексты читают. Был вынужден менять школу. Попытка 'гей - реабилитации' с Клэр.");
	}else{
		N("Мои тексты читают. Больше никаких внеклассных часов для меня. Попытка 'гей - реабилитации' с Клэр.");
	}

	N("В каком-то смысле Стокгольмский синдром... Я благодарен за все это.");
...
function Finale_3(message){
	...
	N("Позже в 2010 году Дэн Сэвидж запустил кампанию It Gets Better&trade;");
	N("Мои три истории ... Ложь, Истина, Полу-Истина ... они все, по крайней мере, верны в одном.");
	N("Это становится лучше.");
