...
function What_Happened_2(){
	...
	n("И это только одна из трёх произошедших паршивых вещей.");
	j("Никки...");
	...
	Choose({
		...
		"Нет, это ИХ вина.": function(message){
			...
			n(message);
			n("Они уже прочитали наши сообщения. Всё, что я сказал бы после этого, не смогло бы изменить произошедшее.");
			if($.told_jack!="texts"){
				j("А? Ты не говорил мне, что они тоже читают твои сообщения!");
			}else{
				j("И они застряли в своей старомодной морали, бедняжки.");
				n("Я бы не заходил так далеко, чтобы их жалеть.");
			}
			What_Now();
		},
	...
	Choose({
		"Я собираюсь подпортить планы моих родителей.": function(message){
			...
			if($.told_jack=="texts"){
				...
				n("Таким образом они больше не смогут шпионить за нашими коммуникациями.");
			}else if($.told_jack=="girl"){
				n("Я расскажу обо всём Клэр. Если повезет, она поможет мне.");
			}else{
				n("Я что-нибудь придумаю...");
			}

			What_Now_2();
		},
		"Завтра я посещу школьного психолога.": function(message){
			n(message);

			if($.told_jack=="abuse"){
				n("Как я и обещал. Как ты заставил меня пообещать.");
			}else if($.told_jack=="school"){
				n("То есть, я говорю о своей нынешней школе. Не знаю, как скоро меня переведут.");
			}else{
				n("По крайней мере, он будут кем-то другим, на кого я могу сорваться.");
			}
		...
		"Я выхожу из этого дома.": function(message){
			...
			n("Но всё равно. Я собираюсь попытаться получить стажировку или стипендию в США.");
