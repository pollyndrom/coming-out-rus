...
function Start_Dinner_4(){
	...
	n(". . .");
	m("Это потому, что твой папа почти никогда не бывает дома, не так ли?");
	m("Без сильного мужского образца для подражания, ты пребываешь в замешательстве ...");
...
function My_Fault(){
	...
	Choose({
		...
		"Хватит притворяться, что ты плачешь.": Cry_2,
		"[плачет]": Cry_3
	});
}
...
function Have_You_Had_Sex(){
	...
	Choose({
		...
		"Нет.": function(message){
			n(message);
			m("Пожалуйста, прекрати лгать... Я видела твои тексты...");
...
function Father_Soon(message){
	...
	m("Твой папа задерживается. Наверное, у него был напряжённый день на работе.");
